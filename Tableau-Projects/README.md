📊 Tableau Projects — Visualizaciones de CO₂ y Tendencias Ambientales

Este folder contiene una serie de visualizaciones desarrolladas en Tableau Public con el objetivo de practicar herramientas de análisis visual y explorar patrones ambientales globales. Los proyectos se centran en las emisiones de CO₂ y su evolución a lo largo del tiempo.

📁 Visualizaciones incluidas
1. 🌍 Nivel de CO₂ por región (1960–2010)

Análisis temporal que muestra la evolución de los niveles de CO₂ por región durante 50 años, permitiendo identificar tendencias, periodos críticos y diferencias entre zonas geográficas.

🔗 Visualización en Tableau:
https://public.tableau.com/views/NiveldeCO2porreginde1960a2010/Hoja1?:language=es-ES&:sid=&:redirect=auth&:display_count=n&:origin=viz_share_link

2. 📈 Emisiones de CO₂ (2000–2011)

Dashboard interactivo que permite explorar emisiones anuales con filtrado por país y comparaciones año a año para identificar variaciones significativas.

🔗 Visualización en Tableau:
https://public.tableau.com/views/AnlisisdeemisionesdeC02de2000a2011/Hoja1?:language=es-ES&:sid=&:redirect=auth&:display_count=n&:origin=viz_share_link

3. 🌐 Global Emissions CO₂

Visualización que resume las emisiones globales mediante mapas y gráficos comparativos, permitiendo ver la distribución mundial y contrastes entre regiones.

🔗 Visualización en Tableau:
https://public.tableau.com/views/GlobalEmissionsC02_17617147498320/Hoja1?:language=es-ES&:sid=&:redirect=auth&:display_count=n&:origin=viz_share_link

🧠 Habilidades desarrolladas

Filtros interactivos y controles dinámicos

Tablas y resúmenes por región y año

Mapas coropléticos y comparaciones visuales

Manejo de Marks: color, tamaño, etiquetas y jerarquía visual

🖼️ Capturas de las visualizaciones

![Nivel de CO₂ por región (1960–2010)](https://github.com/JhonatanMontalvo137/portfolio/blob/main/Tableau-Projects/Nivel%20de%20CO2%20por%20regi%C3%B3n%20de%201960%20a%202010.png)

![Nivel de CO₂ por región (1960–2010)](https://github.com/JhonatanMontalvo137/portfolio/blob/main/Tableau-Projects/Nivel%20de%20CO2%20por%20regi%C3%B3n%20de%201960%20a%202010.png)

![Emisiones de CO₂ (2000–2011)](https://github.com/JhonatanMontalvo137/portfolio/blob/main/Tableau-Projects/An%C3%A1lisis%20de%20emisiones%20de%20C02%20de%202000%20a%202011.png)

![Global Emissions CO₂](https://github.com/JhonatanMontalvo137/portfolio/blob/main/Tableau-Projects/Global%20Emissions%20CO2.png)



